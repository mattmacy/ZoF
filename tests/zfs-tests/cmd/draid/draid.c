    ...
    draid_map_t **best_mapp, double *best_ratiop, double *avg_ratiop)
{
	...
	for (int i = 0; i < passes; i++) {
		 ...
		 */
		int error = vdev_draid_generate_map(children,
		    vdev_draid_rand(map_seed), 0, MAP_ROWS_DEFAULT, &map);
		if (error)
			return (error);
...
draid_verify(int argc, char *argv[])
{
	...
	for (uint64_t children = VDEV_DRAID_MIN_CHILDREN;
		...
		error = vdev_draid_lookup_map(children, &seed, &checksum,
		    &nperms);
		if (error) {
			printf("Error vdev_draid_lookup_map() failed: %s\n",
			    strerror(error));
			return (1);
		}
		error = vdev_draid_generate_map(children, seed, checksum,
		    nperms, &map);
		if (error) {
			printf("Error vdev_draid_generate_map() failed: %s\n",
			    strerror(error));
			return (1);
		}
...
draid_table(int argc, char *argv[])
{
	...
	printf("static draid_map_t draid_maps[VDEV_DRAID_MAX_MAPS] = {\n");
