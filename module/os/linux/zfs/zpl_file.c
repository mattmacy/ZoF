...
#define	pin_user_pages get_user_pages_unlocked
#define	unpin_user_pages put_user_pages
#define	unpin_user_page put_user_page

static int
zpl_pin_user_pages(unsigned long start, int nr_pages, unsigned int flags,
    struct page **pages)
{
	int rc, pinned;

	for (pinned = 0; pinned < nr_pages; pinned += rc) {
		rc = pin_user_pages(start + (pinned * PAGE_SIZE),
		    nr_pages-pinned, pages + pinned, flags);
		printk("pin_user_pages(%lx, %d-%d, %x, %p)->%d\n",
		    start, nr_pages, pinned, flags, pages+pinned, rc);
		if (rc < 0)
			goto out;
	}
out:
...
zpl_get_user_bvec(struct iov_iter *to, struct zpl_page_alloc *zpa, int write)
{
	...
	for (bv_idx = i = 0; i < to->nr_segs; i++) {
		...
		int curlen;
		printk("len=%d nr_pages=%d addr=%llx off=%x\n",
		    len, nr_pages, addr, off);
