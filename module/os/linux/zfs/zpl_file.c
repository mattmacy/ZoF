...
zpl_alloc_bvec_pages(struct iov_iter *to, struct zpl_page_alloc *zpa, int user)
{
	...
	if (user) {
		zpa->zpa_pages = kmem_alloc(sizeof (struct page *) * maxcount,
		    KM_SLEEP);
...
static int
zpl_pin_user_pages(unsigned long start, int nr_pages, int flags,
    struct page **pages)
{
	int rc, pinned;

	for (pinned = 0; pinned < nr_pages; pinned += rc) {
		rc = pin_user_pages(start + pinned * PAGE_SIZE, nr_pages, flags,
		    pages + pinned, NULL);
		if (rc < 0)
			goto out;
	}
...
zpl_get_user_bvec(struct iov_iter *to, struct zpl_page_alloc *zpa, int write)
{
	...
	for (bv_idx = i = 0; i < to->count; i++) {
		...
		pages_done = zpl_pin_user_pages((uint64_t)addr,
		    nr_pages, flags, pages);
	...
	switch (type) {
		...
		case ITER_KVEC:
			*issg = (to->count > 1);
			...
			break;
		case ITER_BVEC:
			bvec = kmem_zalloc(sizeof (*bvec) * to->count,
			    KM_SLEEP);
		...
		default:
			spl_panic(__FILE__, __FUNCTION__, __LINE__,
			    "unrecognized iov_iter type %u", to->type);
	}
