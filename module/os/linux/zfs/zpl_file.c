...
#ifdef WANT_ZPL_DEBUG
#define	ZPL_DBG printk
#else
#define	ZPL_DBG(...)
#endif
/*
...
zpl_get_user_bvec(struct iov_iter *to, struct zpl_page_alloc *zpa, int write)
{
	...
	for (bv_idx = i = 0; i < to->nr_segs; i++) {
		...
		addr &= PAGEMASK;
		pages_done = zfs_get_user_pages((uint64_t)addr,
		...
		int curlen;
		ZPL_DBG("len=%d nr_pages=%d addr=%llx off=%x\n",
