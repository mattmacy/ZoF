	...
	while ((ref = list_head(&rc->rc_list))) {
		...
		if (rc->rc_count != number)
			printf("ref_holder: %p ref_number: %lu %s:%d\n",
			    ref->ref_holder, (uint64_t)ref->ref_number,
			    ref->ref_add_file, ref->ref_add_line);
#endif
...
#if !defined(_KERNEL) || defined(__FreeBSD__)
		if (rc->rc_count != number)
			printf("ref_holder: %p ref_number: %lu add %s:%d "
			    "remove %s:%d\n", ref->ref_holder,
			    (uint64_t)ref->ref_number, ref->ref_add_file,
			    ref->ref_add_line, ref->ref_rem_file,
			    ref->ref_rem_line);
#endif
...
int64_t
zfs_refcount_add_many_(zfs_refcount_t *rc, uint64_t number, const void *holder,
...
int64_t
zfs_refcount_add_(zfs_refcount_t *rc, const void *holder, const char *file,
    int line)
{
...
int64_t
zfs_refcount_remove_(zfs_refcount_t *rc, const void *holder, const char *file,
    int line)
{
	...
	if (!found) {
		...
		for (ref = list_head(&rc->rc_list); ref;
			...
			if (ref->ref_holder == new_holder) {
				panic("new_holder %p already in reference "
				    "list with number %lu and old holder %p "
				    "not found\n", new_holder, ref->ref_number,
				    current_holder);
				found = B_TRUE;
				break;
			}
		...
		for (ref = list_head(&rc->rc_list); ref;
		    ref = list_next(&rc->rc_list, ref)) {
			printf("%p : %lu %s:%d\n", ref->ref_holder,
			    ref->ref_number, ref->ref_add_file,
			    ref->ref_add_line);
		}
	}
#endif
