...
typedef struct dbuf_prefetch_arg {
	...
	callback_fn dpa_cb; /* prefetch completion callback */
	void *dpa_arg; /* prefetch completion arg */
} dbuf_prefetch_arg_t;

static void
dbuf_prefetch_fini(dbuf_prefetch_arg_t *dpa)
{
	if (dpa->dpa_cb != NULL)
		dpa->dpa_cb(dpa->dpa_arg);
...
dbuf_prefetch_impl(dnode_t *dn, int64_t level, uint64_t blkid,
    zio_priority_t prio, arc_flags_t aflags, callback_fn cb, void *arg)
{
	blkptr_t bp;
